<template>
    <div>
        ...
        <!-- Действия с постом: Лайк, просмотры, комментарии, редактирование, удаление -->
                ...
                <div class="flex items-center text-gray-700">
                    ...
                    >
                        <path
                            stroke-linecap="round"
                            stroke-linejoin="round"
                            d="M20.25 8.511c.884.284 1.5
                            1.128 1.5 2.097v4.286c0 1.136-.847
                            2.1-1.98 2.193-.34.027-.68.052-1.02.072v3.091l-3-3c-1.354
                            0-2.694-.055-4.02-.163a2.115 2.115
                            0 0 1-.825-.242m9.345-8.334a2.126
                            2.126 0 0 0-.476-.095 48.64 48.64
                            0 0 0-8.048 0c-1.131.094-1.976
                            1.057-1.976 2.192v4.286c0 .837.46
                            1.58 1.155 1.951m9.345-8.334V6.637c0-1.621-1.152-3.026-2.76-3.235A48.455
                            48.455 0 0 0 11.25 3c-2.115
                            0-4.198.137-6.24.402-1.608.209-2.76
                            1.614-2.76 3.235v6.226c0 1.621
                            1.152 3.026 2.76 3.235.577.075
        ...
        <div class="mt-8">
            ...
            <div v-if="auth && auth.user" class="mb-6">
                ...
                <div class="flex flex-col items-start space-y-3">
                    ...
                    <p v-if="errors['comment.content']" class="text-red-500 text-sm">
                        <span
                            v-for="(error, index) in errors['comment.content']"
                            :key="index"
                        >
                            {{ error }}<br />
                        </span>
            ...
            <div v-if="post.comments_count" class="flex items-center mb-4">
                ...
                >
                    Comments
                    <span
                        class="ml-2 inline-block bg-white text-green-500 font-semibold rounded-full px-2"
                    >
                        {{ post.comments_count }}
                    </span>
                </button>
...
export default {
    ...
    methods: {
        ...
        handleEditComment({ commentId, newContent }) {
            ...
            // Добавляем метод PATCH для method spoofing
            formData.append('_method', 'patch');
            // Добавляем данные, которые нужно обновить
            formData.append('content', newContent);

            axios
                .post(route("admin.comments.update", commentId), formData)
                .then((response) => {
                    this.updateCommentInList(commentId, response.data);
                })
                .catch((error) => {
