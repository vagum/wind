...
==== 24 Telescope Cache Mapper Modal Computed ====
...
Что сделал:
1) Кеши для индекса постов и фильтров.
2) сброс кешев постов при апдейте, удалении или создании постов
3) протыкал всё везде и прооптимизировал в телескопе на запросы 
   к бд. сейчас 0 дублирующихся запросов везде, кроме таблицы
   с юзерями. у юзерей телескоп пишет дубль потому как смотрит
  авторизированный пользователь другого пользователя.
